...
uint8_t replyNumFolders;
uint8_t replyCurrentVolume;
uint8_t fileNotFound;
    ...
    if(osMessageQueueGet(mid_MsgQueueMp3Info,&msg_respuesta,NULL,10)== osOK){
      switch(msg_respuesta.tipo){
        case 0: //= del tipo asíncrono
          if(msg_respuesta.info == 0){ //si no hay tarjeta
            modo_actual = MODO_REPOSO;
            msgRGB = 0x03; //ENVIAR PARPADEO RGB ROJO (RGB_NO_SD)
            osMessageQueuePut(mid_MsgQueue_RGB, &msgRGB, 0U, 0U);
            msgPWM = 0x11; //para que pite
            osMessageQueuePut(mid_MsgQueue_PWM, &msgPWM, 0U, 0U);
            osDelay(200);
          }else if (msg_respuesta.info == 1){ //cuando hay tarjeta
            msgRGB = 0x00;
            osMessageQueuePut(mid_MsgQueue_RGB, &msgRGB, 0U, 0U);
            msgPWM = 0x00;
            osMessageQueuePut(mid_MsgQueue_PWM, &msgPWM, 0U, 0U);
          } else if(msg_respuesta.info == 2){
            fileNotFound = 1;
          }
          break;
        //case1&2 no hace nada
        case 0x42:
        ...
        case 0x4c:
          ...
          break;
        case 0x4F:
          replyNumFolders = msg_respuesta.info;
          break;
        case 0x43:
          ...
          case EV_JOY:
            ...
            switch(msgJoy){
              case 0x01:                                                        // UP: siguiente carpeta (1º canción)
                msgMP3.tipo = 0x00;
                msgMP3.com = 0x0F;
                msgMP3.dat1 = file<replyNumFolders ? file+1 : 0x01;       //puedo ponerlo aquí?? mejor fuera??
                msgMP3.dat2 = 0x01;
                osMessageQueuePut(mid_MsgQueueMp3, &msgMP3, 0, 0);
                file = msgMP3.dat1;
                cancion = msgMP3.dat2;
              ...
              case 0x02:                                                        //RIGHT: siguiente canción (de la misma carpeta)
                ...
                //tiempo_segundos = 0;
                actualizarLCD_modoReproduccion();
                if(fileNotFound == 1){                                               // si no se ha encontrado, sabemos que estamos
                  fileNotFound = 0;
                  msgMP3.tipo = 0x00;
                  msgMP3.com = 0x0F;                                            //al final del folder PROBLEMA RETARDO??
                  msgMP3.dat1 = file;
                  msgMP3.dat2 = 0x01;
                  osMessageQueuePut(mid_MsgQueueMp3, &msgMP3, 0, 0);
                  file = msgMP3.dat1;
              ...
              case 0x04:                                                        //LEFT: canción anterior
                ...
                //tiempo_segundos = 0;
                actualizarLCD_modoReproduccion();
                if(fileNotFound == 1){                                               // si no se ha encontrado, sabemos
                  msgMP3.tipo = 0x00;
                  fileNotFound = 0;
                  msgMP3.com = 0x0F;                                            //que estamos al final del folder
                  msgMP3.dat1 = file;
                  msgMP3.dat2 = 0x01;
                  osMessageQueuePut(mid_MsgQueueMp3, &msgMP3, 0, 0);
                  file = msgMP3.dat1;
              ...
              case 0x05:                                                        //CENTER (CORTO)
                if (replyMP3status == 0x01){                                    //Si esta "Runeando"
                  msgMP3.tipo = 0x00;
                  ...
                  msgRGB = 0x02; //AZUL
                  osMessageQueuePut(mid_MsgQueue_RGB, &msgRGB, 0U, 0U);
                  msgPWM = 0X02; //PWM_PAUSE
                  osMessageQueuePut(mid_MsgQueue_PWM, &msgPWM, 0U, 0U);
                  actualizarLCD_modoReproduccion();
                } else if (replyMP3status == 0x02){                             //si está pausado
                  msgMP3.tipo = 0x00;
...
void actualizarLCD_modoReposo (void){
  msgLCD.linea = 0;
  sprintf(msgLCD.texto, "SBM 2025   T:%.1f C",msgTemp);
...
//  uint8_t sec = tiempo_segundos % 60;
  ...
  osMessageQueuePut(mid_MsgQueueLCD, &msgLCD, 0, 0);
  msgLCD.linea = 2;
  sprintf(msgLCD.texto, "      T   ");
  osMessageQueuePut(mid_MsgQueueLCD, &msgLCD, 0, 0);
}

void actualizarLCD_modoHora(void){
  msgLCD.linea = 0;
  sprintf(msgLCD.texto, "HORA   T:%.1f C",msgTemp);
  osMessageQueuePut(mid_MsgQueueLCD, &msgLCD,0,0);
  msgLCD.linea = 2;
